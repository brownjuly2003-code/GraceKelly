...
That command verifies `snapshot_sha256` when present and reports manifest details such as `manifest_status`, `selection_status`, field-level manifest verification statuses, `selection`, `task_count`, `step_count`, `event_count`, `exported_task_ids`, `missing_task_ids`, `input_size_bytes`, and `import_ready` without requiring database connectivity.
