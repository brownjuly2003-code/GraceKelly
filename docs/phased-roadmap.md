...
Remaining:
- **Browser adapter non-deterministic selection for the `"Best"` alias** — DOM recon completed 2026-04-23 (batch-87, decision = `SELECTOR_DISAMBIGUATE`, see `docs/perplexity-dom-recon.md#best-alias-recon-2026-04-23`). Scoped-menu-search fix landed in `f430d39`; live smoke validation pending before this item can be closed. Nested-text-node risk noted in recon — if scoped+first-match still mis-picks under load, anchor selection to `[role="menuitemradio"]` with exact child text (not partial `has-text`).
