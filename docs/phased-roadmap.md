...
- **batch-87 BROWSER-scoped-menu-search**: `PlaywrightBrowserAutomation.select_model` now tries menu-scoped option lookup (`model_menu_candidates` → `model_menu_item_selector` role-filter) before falling back to the legacy global `get_by_role/text` path; `model_menu_candidates` extended with `[role="menu"]`; `details["option_lookup_source"]` records which path matched. Pattern ported from `Perplexity_Orchestrator2/browser/model_selection.py:182-240` (`f430d39`)
