...
[ ] Harden Playwright model verification and logged-in response extraction beyond the current best-effort selectors. Current blocker: the authenticated UI drifted away from the previously observed model picker, so explicit model selection cannot yet be re-stabilized.
