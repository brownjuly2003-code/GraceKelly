...
- Overall state: normalized task/step/event contracts, profile-aware readiness, packaged PostgreSQL migration tooling, and browser-automation adapter boundaries are implemented in code; the in-memory path is green, the live PostgreSQL path has explicit validation and integration hooks, the browser execution path is now exercisable end-to-end through a scripted backend, model operational hints are now partially enforced in runtime through per-model timeouts and in-process concurrency limits, readiness now exposes execution-plane saturation details for operators, step events carry adapter diagnostics for task-level forensics, final task events now preserve aggregate routing context, and operators can list and triage recent tasks with adapter, requested-model, winning-model, and short-circuit summaries
- Overall state: normalized task/step/event contracts, profile-aware readiness, packaged PostgreSQL migration tooling, and browser-automation adapter boundaries are implemented in code; the in-memory path is green, the live PostgreSQL path has explicit validation and integration hooks, the browser execution path is now exercisable end-to-end through a scripted backend, model operational hints are now partially enforced in runtime through per-model timeouts and in-process concurrency limits, readiness now exposes execution-plane saturation details for operators, step events carry adapter diagnostics for task-level forensics, final task events now preserve aggregate routing context, operators can list and triage recent tasks with adapter, requested-model, winning-model, and short-circuit summaries, and the blocking sync execution path is now offloaded from async HTTP routes.
- Overall state: normalized task/step/event contracts, profile-aware readiness, packaged PostgreSQL migration tooling, and browser-automation adapter boundaries are implemented in code; the in-memory path is green, the live PostgreSQL path has explicit validation and integration hooks, the browser execution path is now exercisable end-to-end through a scripted backend, model operational hints are now partially enforced in runtime through per-model timeouts and in-process concurrency limits, readiness now exposes execution-plane saturation details for operators, step events carry adapter diagnostics for task-level forensics, final task events now preserve aggregate routing context, operators can list and triage recent tasks with adapter, requested-model, winning-model, and short-circuit summaries, the blocking sync execution path is now offloaded from async HTTP routes, and best-effort event persistence failures now leave a warning in logs instead of disappearing silently.
- Overall state: normalized task/step/event contracts, profile-aware readiness, packaged PostgreSQL migration tooling, and browser-automation adapter boundaries are implemented in code; the in-memory path is green, the live PostgreSQL path has explicit validation and integration hooks, the browser execution path is now exercisable end-to-end through a scripted backend, model operational hints are now partially enforced in runtime through per-model timeouts and in-process concurrency limits, readiness now exposes execution-plane saturation details for operators, step events carry adapter diagnostics for task-level forensics, final task events now preserve aggregate routing context, operators can list and triage recent tasks with adapter, requested-model, winning-model, and short-circuit summaries, the blocking sync execution path is now offloaded from async HTTP routes, best-effort event persistence failures now leave a warning in logs instead of disappearing silently, and PostgreSQL connect timeout is now explicit in config instead of relying on driver defaults.
