...
- 2026-03-18: PostgreSQL durability had validation tooling, but no built-in export path for operator snapshots or low-friction backups. Decision: add a JSON export CLI now, scoped to task/step/event data plus schema-health metadata, and leave full restore strategy for a later phase.
