...
[ ] Continue hardening Playwright logged-in response extraction and selector diagnostics beyond the current composer-scoped model-button path. Explicit browser model selection is live-verified again; the remaining work is resilience, not rediscovery.
