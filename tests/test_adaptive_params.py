...
class TestAdaptiveParams(unittest.TestCase):
    ...
    def test_creative_loose(self) -> None:
    ...
    def test_math_params(self) -> None:
    ...
    def test_all_positive_max_rounds(self) -> None:
        for tt in TaskType:
            params = get_adaptive_params(tt)
            self.assertGreater(params.max_rounds, 0)

    def test_all_positive_min_responses(self) -> None:
        for tt in TaskType:
            params = get_adaptive_params(tt)
            self.assertGreater(params.min_responses, 0)

    def test_consensus_target_in_range(self) -> None:
        for tt in TaskType:
            params = get_adaptive_params(tt)
            self.assertGreater(params.consensus_target, 0.0)
            self.assertLessEqual(params.consensus_target, 1.0)

    def test_default_params_structure(self) -> None:
