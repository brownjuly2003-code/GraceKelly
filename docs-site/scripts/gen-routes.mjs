// Walk src/gracekelly/main.py and src/gracekelly/api/routes/*.py and emit
// an MDX page listing every FastAPI route (path, methods, source file).
// Regex-based to avoid importing the heavy GraceKelly app at build time.
...
async function main() {
  ...
  if (existsSync(ROUTES_DIR)) {
    ...
    for (const e of entries) {
      if (!e.isFile() || extname(e.name) !== '.py' || e.name.startsWith('_')) continue;
      all.push(...(await listRoutesInFile(join(ROUTES_DIR, e.name))));
    }
  }
...
<Aside type="tip" title="Source">
  Generated from <code>src/gracekelly/main.py</code> and
  <code>src/gracekelly/api/routes/*.py</code> via regex on FastAPI route
  decorators. Re-run <code>npm run dev</code> or <code>npm run build</code>
  to refresh.
</Aside>
`;
